...
return array('title' => MODULE_PAYMENT_IK_TEXT_DESCRIPTION);
    ...
    if($order->info['currency'] == 'RUR'){
      ...
      $curr = $order->info['currency'];
    }

    $arg = array(
    'ik_cur'=>$curr,
    'ik_co_id'=>MODULE_PAYMENT_IK_SHOP_ID,
    'ik_pm_no'=>intval($OrderID),
    'ik_am'=>round($order->info['total'],2),
    'ik_desc'=>'#'.intval($OrderID),
    );

    $dataSet = $arg;
    ksort($dataSet, SORT_STRING);
    array_push($dataSet, MODULE_PAYMENT_IK_SECRET_KEY);
    $signString = implode(':', $dataSet); 
    $sign = base64_encode(md5($signString, true));

    $process_button_string = os_draw_hidden_field('ik_co_id', MODULE_PAYMENT_IK_SHOP_ID) .
    os_draw_hidden_field('ik_cur', $curr) .
    os_draw_hidden_field('ik_am', $order->info['total']) .
    os_draw_hidden_field('ik_pm_no', $OrderID) .
    os_draw_hidden_field('ik_desc','#'.intval($OrderID)) .
    os_draw_hidden_field('ik_sign', $sign);
    ...
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++)
    {
     if (STOCK_LIMITED == 'true')
     {
      if (DOWNLOAD_ENABLED == 'true')
      {
       ...
       WHERE 
       p.products_id = '".os_get_prid($order->products[$i]['id'])."'
       ";
...
function install()
{
  ...
  os_db_query("insert into ".TABLE_CONFIGURATION." (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_IK_TEST_KEY', '', '6', '3', now())");
  ...
  os_db_query("insert into ".TABLE_CONFIGURATION." (configuration_key, configuration_value, configuration_grou  p_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_IK_ZONE', '0', '6', '5', 'os_get_zone_class_title', 'os_cfg_pull_down_zone_classes(', now())");
